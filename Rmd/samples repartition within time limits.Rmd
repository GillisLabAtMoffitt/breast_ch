...
•	Chemotherapy only:
o	Pre-sample must be closest to drug start from -365 days to + 5days 
o	Sequential sample must be within a year but after stop date

•	Radiation only:
o	Pre-sample must be closest to drug start from -365 days to + 5days 
o	Sequential sample must be within a year but 100 days after radiation

•	Chemotherapy then Radiation:
o	Pre-sample must be closest to drug start from -365 days to + 5days 
o	Sequential sample T2  -> take all
o	choose T2 closest to radiation start (if multiple samples)
o	Sequential sample T3 must be within a year but 100 days after radiation
...
blood_patients %>% 
  ...
  geom_bar()
```
We have `r blood_patients %>% filter(blood_bf_chemo == "Yes") %>%  filter(is.na(chemotherapy_end_date1_1)) %>% nrow()` missing stop date.
...
blood_patients %>% 
  ...
  labs(x="First regimen duration (in days)",
       ...
       title = "Patients who had a presample sample before Chemotherapy") +
...
blood_patients2 <- blood_patients %>% 
  ...
  mutate(interval_presample_hormone = case_when(
    closest_hormone_presample_date == specimen_collection_date  ~ 
      abs(interval(start = closest_hormone_presample_date, end = hormone_therapy_start_date_1) /
                 duration(n = 1, units = "days"))
  )) %>%
...
tbl1_2 <- blood_patients2 %>% filter(had_chemo == "Yes") %>% 
  filter(is.na(had_good_seqsample_Achemorad), seq_sample_chemo == "Yes") %>%
...
blood_patients2 %>%
  ...
  # scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Chemotherapy to T2 (in days)", 
       y="Number of Patient",
