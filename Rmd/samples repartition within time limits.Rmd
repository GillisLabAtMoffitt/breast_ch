...
```{r}
blood_patients2 %>% 
  filter(!is.na(seqsample_id_chemo) & is.na(had_good_seqsample_Achemorad)) %>% 
...
sample_after_chemo %>%
  select(mrn, deidentified_patient_id, time_chemo_seqsample, sequential_sample_count, chemotherapy_drug_1, chemotherapy_drug_2) %>% 
  group_by(deidentified_patient_id) %>% 
  mutate(total_seq_sample = n()) %>% 
  ungroup() %>% 
  filter(total_seq_sample != 1) %>% 
  arrange(deidentified_patient_id, sequential_sample_count, time_chemo_seqsample)
```
...
sample_after_rad %>%
  select(mrn, deidentified_patient_id, time_rad_seqsample, sequential_sample_count, chemotherapy_drug_1) %>% 
  group_by(deidentified_patient_id) %>% 
  mutate(total_seq_sample = n()) %>% 
  ungroup() %>% 
  filter(total_seq_sample != 1) %>% 
  arrange(total_seq_sample, time_rad_seqsample)
```
...
hor <- hormone_patients %>%
  filter(!is.na(presample_id_hormone) | !is.na(seqsample_id_hormone)) %>%
  select(mrn, deidentified_patient_id,
         presample_id_hormone,
         presample_date_hormone,
...
write_csv(hor, "Samples list hormonetherapy group in breast patients 03112022.csv")
```
