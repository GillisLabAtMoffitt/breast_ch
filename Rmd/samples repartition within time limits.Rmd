...
blood_patients <- 
  read_rds("/Users/colinccm/Documents/GitHub/Gillis/breast_ch/blood_patients updated 02012022.rds")
...
- I included the sample BEFORE and CLOSEST to treatment which were collected from 365 days before treatment start to 5 days after treatment start.  
- I included the samples AFTER treatment collected within 365 days after treatment start (365 days after Radiation for group `Chemotherapy then Radiation`.

-Christelle to see how many patients are added with the 5-days after chemo/radiation start to see if we should set it back to 0
-Stop date (chemo only)
--100 days after start date
--Or after latest stop date
---Or after latest stop date +365 days
--Christelle to summarize sample times in context of latest stop date
---Need to decide how many days after stop date to include for sequential sample
-When there are 3 samples for chemo+radiation
--2nd sample
---Need it to be after chemo stop
---AND choose sample closest to radiation start (if multiple samples)
-For patients with radiation only
-Pre-sample
---Up to 1 year before chemo/radiation start would keep most patients – for pre-sample
---If multiple, take closest to radiation start
--Post-sample
---Idea: 100 days after first start date, but within a year
```{r}
...
# blood_patients <- breast_dna %>% left_join(., Treatment, by = "mrn") %>% 
  ...
  mutate(blood_bf_rad = case_when(
    specimen_collection_date <= (radiation_start_date_1 + days(5)) &
...
blood_patients2 <- blood_patients %>% 
  ...
  mutate(seq_sample_chemorad = case_when(
    presample_chemorad_within_limits == "Yes" &
      ...
      specimen_collection_date > second_chemorad_date &
  ...
  mutate(seqsample_id_Achemo = case_when(
    seq_sample_Achemo_Brad == "Yes"                                 ~ sample_id
  )) %>% 
...
# I. Table patients with Blood before and after treatment
```{r}
...
```{r}
sample_before_chemo <- blood_patients2 %>%
...
sample_after_chemo <- blood_patients2 %>%
  ...
  ungroup() %>%
  select(deidentified_patient_id, sequential_sample_count, time_chemo_seqsample)
...
```{r}
blood_patients2 %>%
  filter(!is.na(seqsample_id_chemorad)) %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
