...
* Hormonetherapy:  
  ...
  + Sequential sample T2 must be within a year but 83 days after hormone (match the average duration of chemo)  
     + also needs to be before chemo/rad or no chemo/rad was given
...
blood_patients2 <- blood_patients %>% 
  ...
  mutate(seq_sample_hormone = case_when(
    presample_hormone_within_limits == "Yes" &
      blood_bf_hormone == "No" &
      specimen_collection_date >= (hormone_therapy_start_date_1 + days(83)
                                  ) &
...
blood_patients2 %>% 
  ...
  mutate(regimen_duration = interval(start = chemotherapy_start_date_1, end = chemotherapy_end_date1_1)/
           duration(n=1, units = "days")) %>% 
  select(regimen_duration) %>% 
  tbl_summary(type = list(regimen_duration ~ "continuous"))
blood_patients2 %>% 
  filter(!is.na(seqsample_id_Achemorad)) %>% 
  mutate(regimen_duration = interval(start = chemotherapy_start_date_1, end = chemotherapy_end_date1_1)/
           duration(n=1, units = "days")) %>% 
...
#   fill(everything(), .direction = "updown") %>%
#   distinct()
# write_csv(a, "Samples list chemotherapy group in breast patients 02242022.csv")
```
...
#   distinct()
# 
# write_csv(a, "Samples list radiotherapy group in breast patients 02242022.csv")
```
<br>
<br>
...
#   distinct()
# 
# write_csv(a, "Samples list chemotherapy then radiotherapy group in breast patients 02242022.csv")
```
<br>
<br>
...
hormone_patients <- hormone_patients %>% 
  ...
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  mutate(hormone_stop_T2 = interval(start = hormone_therapy_end_date1_1, end = seqsample_date_Achemo)/
             duration(n=1, units = "days")) %>% 
  mutate(ID = dense_rank(deidentified_patient_id))
...
df <- hormone_patients %>%
  select(deidentified_patient_id, ID,
         ...
         hormone_stop_T2,
         seqsample_before_2nd_regimen_hormone,
...
#   distinct()
# 
# write_csv(a, "Samples list hormonetherapy group in breast patients 02242022.csv")
```

# Done!!!!!

<font size="10"> `r emo::ji("partying_face")` `r emo::ji("party_popper")` `r emo::ji("clinking_glasses")`</font> 
