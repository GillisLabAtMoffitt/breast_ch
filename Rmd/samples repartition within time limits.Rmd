...
-	Sequential sample must be within a year but after stop date  

•	Radiation only:  
-	Pre-sample must be closest to drug start from -365 days to day 0  
...
blood_patients2 %>% 
  ...
  geom_bar()
```
We have `r blood_patients2 %>% filter(!is.na(seqsample_id_chemo)) %>%  filter(is.na(chemotherapy_end_date1_1)) %>% nrow()` missing stop date.
...
blood_patients2 %>% 
  filter(str_detect(deidentified_patient_id,
                    paste0(chemo_patients$deidentified_patient_id, 
                           collapse = "|"))) %>% 
  select(deidentified_patient_id, 
