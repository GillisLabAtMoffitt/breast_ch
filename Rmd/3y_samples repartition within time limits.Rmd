...
* Chemotherapy only:  
  ...
  + Sequential sample must be within 3 year from start date but after chemo stop date ~~and before new drugs/radiation~~  
     + also needs to be before rad or no rad was given  
...
blood_patients2 <- blood_patients %>%
  ...
  mutate(seq_sample_chemo = case_when(
    presample_chemo_within_limits == "Yes" &
      ...
      specimen_collection_date <= (chemotherapy_start_date_1 + years(3)) & #specimen_collection_date <= chemotherapy_start_date_2 &# days(365)) &
      (specimen_collection_date <= radiation_start_date_1 |
      is.na(radiation_start_date_1))                                ~ "Yes",
    TRUE                                                            ~ "No"
  )) %>%
  ...
  mutate(seq_sample_rad = case_when(
    presample_rad_within_limits == "Yes" &
      ...
      specimen_collection_date <= (radiation_start_date_1 + years(3)) & specimen_collection_date <= radiation_start_date_2 &# days(365)) &
      (specimen_collection_date <= chemotherapy_start_date_1 |
      is.na(chemotherapy_start_date_1))                              ~ "Yes",
    TRUE ~ "No"
  )) %>%
  ...
  mutate(seq_sample_hormone = case_when(
    presample_hormone_within_limits == "Yes" &
      ...
      specimen_collection_date <= (hormone_therapy_start_date_1 + years(3)) & #specimen_collection_date <= hormone_therapy_start_date_2 &# days(365)) &
      (specimen_collection_date <= chemotherapy_start_date_1 |
      is.na(chemotherapy_start_date_1)) &
      (specimen_collection_date <= radiation_start_date_1 |
      is.na(radiation_start_date_1))                                     ~ "Yes",
  ...
  mutate(seq_sample_Achemo_Arad = case_when(
    presample_chemo_rad_within_limits == "Yes" &
      ...
      specimen_collection_date > (radiation_start_date_1 + days(100)) &
      specimen_collection_date <= (radiation_start_date_1 + years(3)) #&
      # (specimen_collection_date <= chemotherapy_start_date_2 & ####### code for 3 years filter
      #    specimen_collection_date <= radiation_start_date_2) # days(365))
    ~ "Yes",
    TRUE                                                            ~ "No"
  )) %>%
