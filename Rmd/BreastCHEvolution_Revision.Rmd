---
title: "Revision - Landmark and true time dependent analysis"
...
# True time dependent cox analysis
All patient are included in the analysis.   
The time periods for the time dependent analysis are patient-specific. Cut-off is done at time of 2nd sample.    
Cond is "Unknown" prior 2nd sample time and then are Mona's value.   
1 patient had a PFS event before 2nd sample. In this case, the patient is included for the first time range (which stops at the PFS event time) but doesn't have a second time range.
...
td_dat <- 
  tmerge(
    data1 = data_timedep |> 
      select(deidentified_patient_id, cond, os_event, 
...
<!--   )) %>%  -->
<!--   filter((Źample_number == 1 & time_range == 1) | -->
...
data_timedep <- data_merge %>% 
  ...
  fill(time_tx_to_postsample_months, .direction = "updown") %>% 
  ungroup() %>% 
  # mutate(pfs_first = case_when(
  #   pfs_time_months < time_tx_to_postsample_months        ~ "pfs < sample"
  # )) %>% 
  filter(sample_number == 1)
...
td_dat <- 
  tmerge(
    data1 = data_timedep |> 
      select(deidentified_patient_id, cond, pfs_event, 
             age_at_sample, tnm_stage_cat,
         er, pr, her2), 
    data2 = data_timedep |> 
      select(deidentified_patient_id, pfs_time_months, cond, pfs_event, 
             time_tx_to_postsample_months, cond), 
    id = deidentified_patient_id, 
    pfs_event = event(pfs_time_months, pfs_event),
    cond_event = tdc(time_tx_to_postsample_months)
    ) %>% 
  mutate(cond = case_when(
    cond_event == 1           ~ cond,
