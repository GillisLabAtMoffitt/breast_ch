...
data_merge <- data_merge %>% 
  ...
  mutate(rfs_time_months = interval(start = date_of_first_corresponding_treatment, end = rfs_date)/
         ...
         .after = rfs_event) #%>% 
  # select(mrn, rfs_date, rfs_event, pfs_event, pfs_date, progression_type, date_of_progression, date_of_last_followup, os_event)
...
### 1. Hazard ratios and 95% confidence intervals
```{r RFS, echo = TRUE, class.source= 'fold-hide'}
tbl1 <- tbl_uvregression(
  data_merge,
  method = coxph,
  y = Surv(time = rfs_time_months, event = rfs_event),
  exponentiate = TRUE,
  add_estimate_to_reference_rows = TRUE,
  include = c("age_at_sample", "tnm_stage_cat",
              "er", "pr", "her2", "status_CH"),
...
tbl2 <- coxph(Surv(time = data_merge$rfs_time_months,
                   event = data_merge$rfs_event) ~ age_at_sample + tnm_stage_cat + er + pr + her2 + status_CH,
              data = data_merge)  %>%
  tbl_regression(exponentiate = TRUE,
                 add_estimate_to_reference_rows = TRUE) %>%
  bold_p(t = .05) %>%
...
### 2. KM
The KM are not ajusted
```{r Survival RFS, fig.height = 7, echo = TRUE, class.source= 'fold-hide'}
ggsurvplot(survfit(Surv(rfs_time_months, rfs_event) ~ status_CH,
                   data=data_merge),
           title = "RFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
...
### 1. Hazard ratios and 95% confidence intervals
```{r HR RFS selection_subset, echo = TRUE, class.source= 'fold-hide'}
tbl1 <- tbl_uvregression(
  selection_subset,
  method = coxph,
  y = Surv(time = rfs_time_months, event = rfs_event),
...
tbl2 <- coxph(Surv(time = selection_subset$rfs_time_months,
                   event = selection_subset$rfs_event) ~ age_at_sample + tnm_stage_cat + er + pr + her2 + cond,
              data = selection_subset)  %>%
  tbl_regression(exponentiate = TRUE,
                 add_estimate_to_reference_rows = TRUE) %>%
  bold_p(t = .05) %>%
...
### 2. KM
```{r RFS selection_subset, fig.height = 7, echo = TRUE, class.source= 'fold-hide'}
ggsurvplot(survfit(Surv(rfs_time_months, rfs_event) ~ cond,
                   data=selection_subset),
           title = "RFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
