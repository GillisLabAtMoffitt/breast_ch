...
blood_patients <- 
  read_rds("/Users/colinccm/Documents/GitHub/Gillis/breast_ch/blood_patients updated 01312022.rds")
...
We focus on the chemotherapy and radiation treatment so the samples are not chosen depending of immunotherapy or hormonetherapy.
- I included the sample BEFORE and CLOSEST to treatment 
- I included ALL the samples AFTER treatment.
...
```{r}
# sample_before_chemo <- blood_patients %>% 
...
# sample_before_hormone <- blood_patients %>% 
...
# sample_before_immuno <- blood_patients %>% 
...
#   facet_zoom(ylim = c(0, 10), zoom.size = 1)
# 
# sample_before_rad <- blood_patients %>% 
...
blood_patients2 <- blood_patients %>% 
  ...
  mutate(presample_within_limits = had_good_sample_chemo) %>% 
  group_by(deidentified_patient_id) %>%
  fill(presample_within_limits, #interval_presample_chemo,
       closest_chemo_presample_date, .direction = "updown") %>%
  mutate(seq_sample_chemo = case_when(
    presample_within_limits == "Yes" &
      blood_bf_chemo == "No" &
      specimen_collection_date > chemotherapy_start_date_1 &
      (specimen_collection_date <= radiation_start_date_1 |
      is.na(radiation_start_date_1))                                ~ "Yes",
  ...
  mutate(after_chemo_seqsample_date = case_when(
    had_good_seqsample_Achemo == "Yes"                           ~ first(specimen_collection_date)
  )) %>% 
...
sample_after_chemo <- blood_patients2 %>%
...
sample_Achemo_Brad %>% 
  ...
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Chemotherapy start to sequential sample (in days)", 
       y="Number of Patient",
...
sample_Achemo_Arad %>% 
  ...
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Radiotherapy to sequential sample (in days)", 
       y="Number of Patient",
...
tbl1_2 <- blood_patients2 %>% filter(had_chemo == "Yes") %>% 
  filter(seq_sample_chemo == "Yes") %>%
...
#   tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl4_2 <- blood_patients2 %>% filter(had_rad == "Yes") %>% 
  filter(seq_sample_rad == "Yes") %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(Radiotherapy = seq_sample_rad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl5_2 <- blood_patients2 %>% filter(had_chemo_rad == "Yes") %>% 
...
df <- blood_patients2 %>% 
  ...
  select(deidentified_patient_id, ID,
         ...
         "number post treatment sample"  = sequential_sample_count, 
         "days from second treatment to sequential sample" =  time_chemrad_seqsample)
color.me1 <- which((df$ID %% 2)  == 1)
# color.me <- which(df$`number post treatment sample` == 2)
# color.me.bold <- which((df$ID %% 2)  == 1)
color.me.bold <- which(df$`number post treatment sample` == 1)
...
```{r}
blood_patients2 %>% filter(had_chemo == "Yes") %>% 
  filter(seq_sample_chemo == "Yes") %>% 
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
