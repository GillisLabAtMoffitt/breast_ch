...
breast_dna2 <- breast_dna1 %>% 
  ...
  mutate(time_prechemo_seq_sample = case_when(
    seq_sample_chemo == "Yes"        ~ (
      interval(start = closest_chemo_presample_date, end = specimen_collection_date) / 
        duration(n= 1, units = "days")
  ))) %>% 
  ...
  mutate(time_prehormone_seq_sample = case_when(
    seq_sample_hormone == "Yes"        ~ (
      interval(start = closest_hormone_presample_date, end = specimen_collection_date) /
        duration(n = 1, units = "days")
  ))) %>% 
  ...
  mutate(time_prerad_seq_sample = case_when(
    seq_sample_rad == "Yes"        ~ (
      interval(start = closest_rad_presample_date, end = specimen_collection_date) / 
        duration(n= 1, units = "days")
  ))) %>% 
...
# Table total samples count before and after treatment
```{r}
tbl1 <- breast_dna2 %>% #filter(had_chemo == "Yes") %>% 
  select(Chemotherapy = blood_bf_chemo) %>% 
  tbl_summary() %>% 
...
breast_dna2 %>% 
  ...
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  ggplot(aes(x =time_prechemo_seq_sample, fill = sequential_sample_count
             ))+
  geom_histogram(binwidth = 30, alpha = 0.9, position = "stack") +
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Chemotherapy Pre Treatment Sample to \nPost Treatment Sample (in days)", 
       y="Number of Patient",
...
df <- breast_dna2 %>% 
  ...
  ungroup() %>% 
  mutate(ID = dense_rank(deidentified_patient_id)) %>% 
  select(ID, "number post treatment sample"  = sequential_sample_count, "days from closest pre treatment sample to post treatment sample" =  time_prechemo_seq_sample)
...
breast_dna2 %>% 
  ...
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  ggplot(aes(x =time_prehormone_seq_sample, fill = sequential_sample_count
             ))+
  geom_histogram(binwidth = 30, alpha = 0.9, position = "stack") +
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
...
df <- breast_dna2 %>% 
  ...
  ungroup() %>% 
  mutate(ID = dense_rank(deidentified_patient_id)) %>% 
  select(ID, "number post treatment sample"  = sequential_sample_count, "days from closest pre treatment sample to post treatment sample" =  time_prehormone_seq_sample)
...
breast_dna2 %>% 
  ...
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  ggplot(aes(x =time_prerad_seq_sample, fill = sequential_sample_count
             ))+
  geom_histogram(binwidth = 30, alpha = 0.9, position = "stack") +
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
...
df <- breast_dna2 %>% 
  ...
  ungroup() %>% 
  mutate(ID = dense_rank(deidentified_patient_id)) %>% 
  select(ID, "number post treatment sample"  = sequential_sample_count, "days from closest pre treatment sample to post treatment sample" =  time_prerad_seq_sample)
