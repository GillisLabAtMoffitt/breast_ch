...
As we talked, I included the sample closest to treatment when multiple samples were available BEFORE treatment and I included ALL the samples AFTER treatment.
...
arrange(mrn, deidentified_patient_id) %>% 
  select(mrn, deidentified_patient_id, specimen_collection_date, #sample_lag, has_a_good_sample, has_a_good_seq_sample,
         ...
         # "blood_bf_immuno", "blood_bf_hormone", 
         "blood_bf_chemo_rad",
...
breast_dna2 <- breast_dna1 %>% 
  ...
  fill(had_good_sample_chemo, interval_presample_chemo,
  ...
  mutate(interval_prepost_sample_chemo =
  ...
  # 
  # group_by(deidentified_patient_id) %>% 
  # fill(had_good_sample_hormone, interval_presample_hormone,
  ...
  #                duration(n = 1, units = "days"))) %>%
  # group_by(deidentified_patient_id) %>% 
  # fill(had_good_sample_immuno, interval_presample_immuno,
  ...
  mutate(interval_presample_rad =
           abs(interval(start = closest_rad_presample_date, end = radiation_start_date_1) /
                 duration(n = 1, units = "days"))) %>%
  group_by(deidentified_patient_id) %>%
  fill(had_good_sample_rad, interval_presample_rad,
  ...
  ################### Variables for sample after C + Rad
  ...
  mutate(interval_presample_chemrad =
           abs(interval(start = closest_chemorad_presample_date, end = first_chemo_rad_date) /
                 duration(n = 1, units = "days"))) %>%
  group_by(deidentified_patient_id) %>%
  fill(had_good_sample_chemrad, interval_presample_chemrad,
  ...
  mutate(second_chemrad_date = first_chemo_rad_date + abs(interval_chemo_rad)) %>%
  mutate(seq_sample_chemrad = case_when(
    had_good_sample_chemrad == "Yes" &
      blood_bf_chemo_rad == "No" &
      specimen_collection_date > second_chemrad_date               ~ "Yes",
    TRUE                                                           ~ "No"
  )) %>%
  # select(deidentified_patient_id, specimen_collection_date, had_good_sample_chemrad, closest_chemorad_presample_date, first_chemo_rad_date,second_chemrad_date,second_chemrad_date, seq_sample_chemrad, chemotherapy_start_date_1, radiation_start_date_1)
  mutate(time_chemrad_seqsample = case_when(
    seq_sample_chemrad == "Yes"        ~ (
      interval(start = second_chemrad_date, end = specimen_collection_date) /
        duration(n= 1, units = "days")
  ))) %>%
  mutate(interval_prepost_sample_chemrad =
           abs(interval(start = closest_chemorad_presample_date, end = specimen_collection_date) /
                 duration(n = 1, units = "days"))) %>%

  ################### Variables for sample after C then Rad
  ...
  mutate(interval_presample_chemo_rad =
           abs(interval(start = closest_chemo_rad_presample_date, end = chemotherapy_start_date_1) /
                 duration(n = 1, units = "days"))) %>%
  # Look for seq sample after C and before R
  group_by(deidentified_patient_id) %>% 
  fill(had_good_sample_chemo_rad, closest_chemo_rad_presample_date, .direction = "updown") %>% 
  ungroup() %>% 
  ...
  # Look for seq sample after C then R
  group_by(deidentified_patient_id) %>%
  fill(had_good_seqsample_Achemo, .direction = "updown") %>%
  ungroup() %>%
  ...
  mutate(interval_prepost_sample_chemo_rad = case_when(
    !is.na(time_rad_2nd_seqsample)                                  ~ abs(
      interval(start = closest_chemo_rad_presample_date, end = specimen_collection_date) /
        duration(n = 1, units = "days")
  ))) %>% 
...
sample_after_chemo <- breast_dna2 %>%
  ...
  ungroup() %>%
  select(deidentified_patient_id, sequential_sample_count, time_chemo_seqsample, everything())
...
sample_after_chemo %>%
  ...
  labs(x="Time from Blood Collection to Chemotherapy (in days)",
       ...
       title = "Control patients who had a sequential sample after Chemotherapy")
...
sample_after_rad %>%
  ...
  labs(x="Time from Blood Collection to Radiotherapy (in days)",
       y="Number of Patient",
       caption = "Each bar represents 100 days",
       title = "Control patients who had a sequential sample after Radiotherapy")

sample_Achemo_Brad <- breast_dna2 %>%
  filter(had_good_seqsample_Achemo == "Yes") %>%
  arrange(deidentified_patient_id, time_chemo_1st_seqsample) %>%
  distinct(deidentified_patient_id, specimen_collection_date, .keep_all = TRUE) %>% 
  group_by(deidentified_patient_id) %>% 
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  select(deidentified_patient_id, sequential_sample_count, time_chemo_1st_seqsample, everything())
...
sample_Achemo_Brad %>% 
  ...
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Chemotherapy to 1st sequential sample (in days)", 
       y="Number of Patient",
       title = "Patients who had a 1st sequential sample after Chemotherapy",
       caption = "Each bar on the top plot represents 30 days") +
  ggforce::facet_zoom(xlim = c(0, 250), zoom.size = 2)


sample_Achemo_Arad <- breast_dna2 %>%
  filter(had_good_seqsample_Achemorad == "Yes") %>%
  arrange(deidentified_patient_id, time_rad_2nd_seqsample) %>%
  distinct(deidentified_patient_id, specimen_collection_date, .keep_all = TRUE) %>% 
  group_by(deidentified_patient_id) %>% 
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  select(deidentified_patient_id, sequential_sample_count, time_rad_2nd_seqsample, everything())
...
sample_Achemo_Arad %>% 
  ...
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Radiotherapy to 2nd sequential sample(in days)", 
       y="Number of Patient",
       title = "Patients who had a 1st sequential sample after Chemotherapy and a \n2nd sequential sample after Radiation",
...
breast_dna2 %>%
  ...
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
  labs(x="Time from Chemotherapy to Radiotherapy for patients who have a 2nd sequential sample(in days)", 
       y="Number of Patient",
       caption = "Each bar represents 100 days")
```
...
tbl1_1 <- breast_dna2 %>% 
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(Chemotherapy = had_good_sample_chemo) %>% 
...
tbl4_1 <- breast_dna2 %>% 
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(Radiotherapy = had_good_sample_rad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl5_1 <- breast_dna2 %>% 
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(`Chemotherapy then Radiation` = had_good_sample_chemo_rad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl6_1 <- breast_dna2 %>% 
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(`C+R or R+C` = had_good_sample_chemrad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
...
tbl1_2 <- breast_dna2 %>% filter(had_chemo == "Yes") %>% 
  filter(seq_sample_chemo == "Yes") %>%
...
#   tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl4_2 <- breast_dna2 %>% filter(had_rad == "Yes") %>% 
  filter(seq_sample_rad == "Yes") %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(Radiotherapy = seq_sample_rad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl5_2 <- breast_dna2 %>% 
  filter(had_good_sample_chemo_rad == "Yes") %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(`Chemotherapy then Radiation` = had_good_seqsample_Achemo) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
tbl6_2 <- breast_dna2 %>%
  filter(seq_sample_chemrad == "Yes") %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(`C+R or R+C` = seq_sample_chemrad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}"))
# tbl6 <- breast_dna2 %>% #filter(had_treatment == "Yes") %>% 
...
tbl5_3 <- breast_dna2 %>% filter(had_chemo_rad == "Yes") %>% 
  filter(had_good_sample_chemo_rad == "Yes") %>%
  distinct(deidentified_patient_id, .keep_all = TRUE) %>% 
  select(`Chemotherapy then Radiation` = had_good_seqsample_Achemorad) %>% 
  tbl_summary(statistic = list(all_categorical() ~ "{n}")) %>% 
  modify_header(update = list(
  stat_0 ~ '**Patients with Sequential Blood after Chemo and Rad**'
))
...
tbl_stack(list(tbl_s1, tbl_s4, tbl_s5, tbl_s6)) %>%
...
# tbl5 <- breast_dna2 %>% filter(had_chemo_rad == "Yes") %>% 
#   filter(seq_sample_chemrad == "Yes") %>%
#   select(`Chemotherapy+Radiation` = seq_sample_chemrad) %>% 
#   tbl_summary(statistic = list(all_categorical() ~ "{n}"))
# # tbl6 <- breast_dna2 %>% #filter(had_treatment == "Yes") %>% 
# #   filter(seq_sample_treatment == "Yes") %>%
# #   select(`All Treatment` = seq_sample_treatment) %>% 
...
breast_dna2 %>% 
  ...
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  ggplot(aes(x =interval_prepost_sample_chemo, fill = sequential_sample_count
             ))+
  geom_histogram(binwidth = 30, alpha = 0.9, position = "stack") +
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
...
color.me1 <- which((df$ID %% 2)  == 1) # strips to distinguish patients
...
df <- breast_dna2 %>% 
  ...
  select(deidentified_patient_id, ID,
         "days from pre sample to rad" = interval_presample_chemo_rad,
         "number post treatment sample"  = sequential_sample_count, 
         "days from chemo to 1st sequential sample"  = time_chemo_1st_seqsample,
         "days from rad to 2nd sequential sample" =  time_rad_2nd_seqsample,
         "days from 1st to 2nd sequential sample"  = interval_prepost_sample_chemo_rad)
...
color.me.seq2 <- which(!is.na(df$`days from rad to 2nd sequential sample`))
```
...
df %>% 
  ...
  # kableExtra::row_spec(color.me, background = "#eee") %>%
  kableExtra::row_spec(color.me1, background = "#eee") %>%
  kableExtra::row_spec(color.me.pre, color = "blue") %>%
  kableExtra::row_spec(color.me.seq1, color = "red") %>%
  kableExtra::row_spec(color.me.seq2, color = "green")
```
<br>
...
```{r, fig.width=12, fig.height=9}
breast_dna2 %>% 
  filter(!is.na(time_chemrad_seqsample)) %>% 
  group_by(deidentified_patient_id) %>% 
  mutate(sequential_sample_count = factor(row_number(deidentified_patient_id))) %>% 
  ungroup() %>% 
  ggplot(aes(x =interval_prepost_sample_chemrad, fill = sequential_sample_count
             ))+
  geom_histogram(binwidth = 30, alpha = 0.9, position = "stack") +
  scale_fill_viridis(discrete=T)+
  theme_minimal(base_size = 14) +
...
df <- breast_dna2 %>% 
  ...
  # mutate(ID = dense_rank(deidentified_patient_id)) %>% 
  select(deidentified_patient_id, 
         "days from pre sample to chemrad" = interval_presample_chemrad,
         "number post treatment sample"  = sequential_sample_count, 
         "days from second treatment to sequnetial sample" =  time_chemrad_seqsample)
color.me1 <- which((df$ID %% 2)  == 1)
# color.me <- which(df$`number post treatment sample` == 2)
# color.me.bold <- which((df$ID %% 2)  == 1)
color.me.bold <- which(df$`number post treatment sample` == 1)
