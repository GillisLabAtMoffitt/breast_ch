...
df <- breast_dna2 %>% 
  ...
  # mutate(ID = dense_rank(deidentified_patient_id)) %>% 
  select(deidentified_patient_id, 
         "days from pre sample to rad" = interval_presample_chemrad,
         "number post treatment sample"  = sequential_sample_count, 
         "days from treatment to sequnetial sample" =  time_chemrad_seqsample)
